...
class APICollector:
        ...
        def getData(self, param):
            response = None
            if param > 1:
                  response = requests.get(f'http://127.0.0.1:8000/generate_purchases/{param}').json()
            else:
                 response = requests.get('http://127.0.0.1:8000/generate_purchase').json()
            return response
        ...
        def fileName(self):
             ...
             return f"api/api-response-purchase{match[0]}.parquet"